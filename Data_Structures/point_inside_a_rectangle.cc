...
{
	string result;
	if (p.x > r.xmin and p.x < r.xmax and p.y > r.ymin and p.y < r.ymax) result = "inside";
	else if ((p.x == r.xmin and (p.y >= r.ymin and p.y <= r.ymax)) or (p.y == r.ymin and (p.x >= r.xmin and p.x <= r.xmax)) or (p.x == r.xmax and (p.y >= r.ymin and p.y <= r.ymax)) or (p.y == r.ymax and (p.x >= r.xmin and p.x <= r.xmax))) result = "border";
	else result = "outside";
	return result;
...
int main()
{
	...
	for (int i = 0; i < n; ++i) {
		...
		string position = containtment(p,r);
		cout << position << endl;
	}
}
